...
我们有一个自动安装脚本。使用它可以简化安装。**但是，它还没有完成，而且充满了错误。欢迎提出PR。**
...
目前它已经在 Termux 和其中的 Kali 容器测试通过。一旦它真正稳定下来并在大量用户的环境中通过测试，我们将把它设置为推荐的安装方法。

> 警告：安卓10以上的设备对权限控制有改动，目前该脚本没有适配，它们的用户必须手动安装！

## 手动安装

如果上面的自动安装脚本不工作，那么请尝试以下步骤。
